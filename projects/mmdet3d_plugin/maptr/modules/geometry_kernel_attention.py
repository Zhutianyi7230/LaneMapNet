    ...
    def forward(self,
        ...
        for j in range(bs):
            for i in range(self.num_cams):
                index_query_per_img = indexes[i][j]
                slots[j, index_query_per_img] = slots[j, index_query_per_img] + queries[j,
...
class GeometryKernelAttention(BaseModule):
    ...
    def forward(self,
        ...
        value = value.view(bs, num_value, self.num_heads, -1) #torch.Size([12, 375, 4, 64])
        ...
        attention_weights = attention_weights.view(bs, num_query,
                                                   self.num_heads,
                                                   self.num_levels,
                                                   self.num_points)#torch.Size([12, 7146, 4, 1, 15])
        if reference_points.shape[-1] == 2:
            ...
            with torch.no_grad():
                ...
                # (bs, q, nz, l, k, xy)
                sampling_locations = (
                    reference_points[:, :, :, :, None, :] + offsets).round().long() #torch.Size([12, 7146, 4, 1, 15, 2])
