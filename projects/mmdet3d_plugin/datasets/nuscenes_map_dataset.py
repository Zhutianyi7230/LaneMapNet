...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def __init__(self,
                 ...
                 noise_std=0,
                 n_control=3,###
                 polyrnn_feat_side=[60,30],###
                 line_label_root=None,####
                 loc_dict_path=None,###
        ...
        self.polyrnn_feat_side = polyrnn_feat_side
        # self.nuscenes = NuScenes(version='v1.0-trainval',dataroot=self.data_root)
        self.map_apis = { location : NuScenesMap(os.path.expandvars(self.data_root), location) 
             for location in nusc_utils.LOCATIONS }
        self.line_label_root = line_label_root
        self.loc_dict_path = loc_dict_path
        self.loc_dict = np.load(self.loc_dict_path,allow_pickle=True)
        self.class_dict_path = class_dict_path
        self.class_dict = np.load(self.class_dict_path,allow_pickle=True)
    ...
    def get_line_labels(self, sample_token, road):
        all_classes = self.class_dict.item().get(sample_token)
        road_cls = all_classes[road-1]# 1 for lane_center; 2 for lane_connector
        return road_cls
        

    def get_line_orientation(self,sample_token, road,img_centers,loc,vis_mask, custom_endpoints=None):
        scene_map_api = self.map_apis[loc]
        all_lines_tokens = self.lines_dict[loc]#该loc下所有lane和lane_connector的token
        ...
        # m = np.zeros([400,200],dtype=np.uint16)#(400，200)
        # m[img_centers==road] = 255
        # m = m.astype(np.uint8)
        # # cv2.circle(m, (my_col_id[0], my_row_id[0]), 3, (255, 0, 0), thickness=5)
        # # cv2.circle(m, (my_col_id[1], my_row_id[1]), 3, (255, 0, 0), thickness=5)
        # cv2.imwrite('m.jpg',m)
        # import pdb;pdb.set_trace()
        ...
        outgoing_token = scene_map_api.get_outgoing_lane_ids(token)#寻找后继的line的token
        outgoing_id = []
        ...
        incoming_token = scene_map_api.get_incoming_lane_ids(token)#寻找前继的line的token
        incoming_id = []
    ...
    ##load labels for STSU  
    def load_line_labels(self,input_dict):
        
        token = input_dict['lidar_token']
        location = input_dict['map_location']
        
        label_path = os.path.join(self.line_label_root, token + '.png')
        orig_img_centers = cv2.imread(label_path, cv2.IMREAD_UNCHANGED )#(400, 200)
        # cv2.imwrite('orig_img_centers.jpg',orig_img_centers)###
        ...
        H = 400
        W = 200
        vis_mask = np.ones((int(H), int(W)), dtype=bool)
        ...
        roads = np.unique(img_centers)[1:]#array([  80,  365,  498,  645,  737,  771,  877,  933, 1188, 1197, 1286,
    #    1337, 1398, 1447, 1475, 1571, 1828, 1891, 1912, 1914], dtype=uint16)
        outgoings = []
        incomings = []
        coeffs_list = []
        to_remove=[]
        ...
        for k in range(len(roads)):
            ...
            locs = np.where(sel)#出现roads[k]数值的点所在坐标
            ...
            inc, out, endpoint, endpoint_id = self.get_line_orientation(token,roads[k],img_centers,location,vis_mask,custom_endpoints=None)
            ##endpoint和points好像不一致，查看后发现endpoint是错的.那就不用endpoint了
            
            label = self.get_line_labels(token, roads[k])
            labels.append(label)
            ...
            #fin_res=array([[0.68765952, 0.34693878],
                        #    [0.71589144, 0.17346939],
                        #    [0.74168437, 0.        ]])
            #end_point= array([[0.685     , 0.35204083],
                        #    [0.74      , 0.        ]], dtype=float32)  

            # import pdb;pdb.set_trace()
            # fin_res_torch = torch.from_numpy(fin_res).unsqueeze(0).to(torch.float32)
            # bezierA = bezier.bezier_matrix(n_control=self.n_control,n_int=50)
            # target_points = torch.matmul(bezierA.expand(fin_res_torch.size(0),-1,-1),fin_res_torch)
            # import pdb;pdb.set_trace()
        ...
        if len(to_remove) > 0:
            ...
            for k in to_remove:
                ...
                roads.remove(k)
                #20231107:去除和第k个实例有关的连接关系，并改写成正确的新的连接关系
                #找出k的inc和out
                inc = []
                out = []
                for idx in range(len(outgoings)):
                    if k-1 in outgoings[idx]:
                        inc.append(roads[idx]-1)
                        outgoings[idx].remove(k-1)
                for idx in range(len(incomings)):
                    if k-1 in incomings[idx]:
                        out.append(roads[idx]-1)
                        incomings[idx].remove(k-1)
                #为inc和out建立连接关系
                for ins in inc:
                    outgoings[roads.index(ins+1)].extend(out)
                for ins in out:
                    incomings[roads.index(ins+1)].extend(inc)
        ...
        orig_img_centers,origs,\
        to_return_centers,labels, roads,\
        np.array(coeffs_list), outgoings, incomings,singapore, False, obj_exists
        
    def generate_stsu_targets(self,obj_to_return, center_width_orient,con_matrix,endpoints,mask, bev_mask, orig_img_centers,\
        origs, to_return_centers, labels,roads,coeffs,\
        outgoings, incomings, singapore,problem, obj_exists):
                
        sorted_points = np.copy(coeffs)
        grid_sorted_points = np.reshape(sorted_points,(-1,self.n_control ,2))
        grid_sorted_points[...,0]= np.int32(grid_sorted_points[...,0]*(self.polyrnn_feat_side[1] - 1))
        grid_sorted_points[...,1]= np.int32(grid_sorted_points[...,1]*(self.polyrnn_feat_side[0] - 1))
        ...
        # target['endpoints'] = torch.tensor(endpoints)#20230816注释掉了这一行
    ...
    def prepare_train_data(self, index):
        ...
        origs, to_return_centers, labels,roads,coeffs,\
        outgoings, incomings, singapore,problem, obj_exists = self.load_line_labels(input_dict)

        example_stsu = self.generate_stsu_targets(obj_to_return, center_width_orient,con_matrix,endpoints,mask, bev_mask, orig_img_centers,\
        origs, to_return_centers, labels,roads,coeffs,\
        outgoings, incomings, singapore,problem, obj_exists)
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            lidar_token=info['lidar_data_token'],#
    ...
    def prepare_test_data(self, index):
        ...
        if self.is_vis_on_test:
            example = self.vectormap_pipeline(example, input_dict)
        #example['img'][0].data.shape = torch.Size([6, 3, 480, 800])
        #example['img_metas'][0].data.keys()
        ...
        return self.union2one_test(data_queue)
        # return example
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if data is None:
                ...
                continue
            if data['target']['labels'].shape[0]==0:
                idx = self._rand_another(idx)
                continue     
            return data
    def _format_gt(self):
    ...
    def evaluate(self,
        ...
        """
        # stsu_results = [result['pts_stsu'] for result in results]
        bbox_results = [result['pts_bbox'] for result in results]
