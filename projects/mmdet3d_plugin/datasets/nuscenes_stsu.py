...
import random
from projects.mmdet3d_plugin.datasets import utils as nusc_utils
...
class NuScenesMapDataset(Dataset):
        ...
        for scene in self.nuscenes.scene:
            ...
            for sample in iterate_samples(self.nuscenes, 
                ...
                for camera in CAMERA_NAMES:
                    self.tokens.append(sample['data']['LIDAR_TOP'])################
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            grid_sorted_points[...,0]= np.int32(grid_sorted_points[...,0]*(self.config.polyrnn_feat_side[1] - 1))
            grid_sorted_points[...,1]= np.int32(grid_sorted_points[...,1]*(self.config.polyrnn_feat_side[0] - 1))
...
#            target['static_mask'] = static_mask
    ...
    def  get_line_orientation(self,sample_token, road,img_centers,loc,vis_mask, custom_endpoints=None, augment=False):
        ...
        try:
            ...
            #ex: road = 365
            #all_ends[road-1,:,0] = array([126, 195], dtype=uint8)
...
#                     logging.error('LINE ' + tok + ' not in lines')
    ...
    def get_object_params(self, token, vis_mask,beta,augment):
        resolution=self.resolution#0.25
        
        objs = self.obj_dict.item().get(token)#(38,9)
        ...
        for obj in objs:
            ...
            reshaped[:,0] = (reshaped[:,0] - self.config.map_extents[0])/(self.config.map_extents[2]-self.config.map_extents[0])#横坐标归一化
            
            if augment:
                reshaped[:,1] = reshaped[:,1] - beta
            
            reshaped[:,1] =  (reshaped[:,1] - self.config.map_extents[1])/(self.config.map_extents[3]-self.config.map_extents[1])#纵坐标归一化
            ...
            if inside:
                ...
                long_side = np.argmax(all_lengths)#最长的一条边
...
#        
    ...
    def load_line_labels(self, token, augment, beta):
        ...
        # label_path = '/mnt/data/nuscenes/nuscenes_stuff2/lines/ee33166a237a49a4997bedfb007c95f9.png'
        orig_img_centers = cv2.imread(label_path, cv2.IMREAD_UNCHANGED )#(400, 200)
        # cv2.imwrite('a.jpg',orig_img_centers)##
        
        # bev_labels = self.load_seg_labels(token, scene_token)#torch.Size([11, 196, 200])
        bev_labels = None
        ...
        if self.work_objects:
            ...
            #obj_to_return.shape = (9,9)
            #center_width_orient.shape = (9,5)  5：中心点xy坐标，angle，最长边，最短边长度
        else:
...
#       
        ...
        np.array(coeffs_list), outgoings, incomings,singapore, False, obj_exists
            ...
            #center_width_orient.shape = (9,5)  5：中心点xy坐标，angle，最长边，最短边长度
            #obj_exists
            #连接性：con_matrix.shape=[17,17],17是当前line的数量
            #np.array(endpoints) 所有line的端点 [17,4]
            #vis_labels：两层vis_masks, torch.Size([2, 196, 200])
            #bev_labels: 10种object的bev分割标签 #torch.Size([11, 196, 200])
            #orig_img_centers： torch.Size([196, 200])
            #np.stack(origs)   ：  (17, 196, 200)
            #to_return_centers.shape= torch.Size([196, 200])
            #labels: torch.ones(17)
            #np.array(coeffs_list).shape = (17,6) 贝塞尔曲线控制点
            #outgoings = [[1824], [1612], [1196, 1913], [1446], [1187, 1336], [1397], [804], [497], [279], [539], [79], [497], [364], [503], [364], [503], [503]]
            #singapore = True
